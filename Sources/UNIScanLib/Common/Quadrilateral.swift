...
public struct Quadrilateral: Codable, Transformable {
  ...
  mutating public func reorganize() {
...
public extension Quadrilateral {
  ...
  func relativeToSize(_ size: CGSize) -> Quadrilateral {
    return Quadrilateral(topLeft: .init(x: topLeft.x / size.width, y: topLeft.y / size.height),
...
extension Quadrilateral {
  ...
  public static func defaultHorizontalQuad(for image: UIImage) -> Quadrilateral {
    let imgSize = image.size
    return Quadrilateral(
...
extension Quadrilateral {
  ...
  static public func quad(_ first: Quadrilateral,
                          significantlyDifferentThen second: Quadrilateral) -> Bool {
...
//    return abs(first.squreValue() - second.squreValue()) >= threshold
  ...
  func squreValue() -> CGFloat {
    ...
    return CGFloat(sqrtf(
      zip(lineLengths, (0...lineLengths.count)
...
func lengthOf(_ line: Line) -> Float {
  return sqrtf(Float(pow((line.p2.x - line.p1.x), CGFloat(2)) + pow((line.p2.y - line.p1.y), CGFloat(2))))
}
