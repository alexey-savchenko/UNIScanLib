...
extension UIImage {
  ...
  func applyingPortraitOrientation() -> UIImage {
    ...
    case .down:
      return rotated(
        by: Measurement(value: Double.pi, unit: .radians),
        options: [.flipOnVerticalAxis, .flipOnHorizontalAxis]
      ) ?? self
    case .left:
      return self
    case .right:
  ...
  func rotated(
    by rotationAngle: Measurement<UnitAngle>,
    ...
    let image = renderer.image { renderContext in
      ...
      let drawRect = CGRect(
        origin: CGPoint(x: -cgImageSize.width / 2.0, y: -cgImageSize.height / 2.0),
        size: cgImageSize
      )
