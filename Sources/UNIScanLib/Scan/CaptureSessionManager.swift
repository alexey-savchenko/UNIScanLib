...
public protocol RectangleDetectionDelegateProtocol: NSObjectProtocol {
  ...
  func captureSessionManager(_ captureSessionManager: CaptureSessionManager, didReceiveImage image: CIImage)
}

extension RectangleDetectionDelegateProtocol {
  func captureSessionManager(_ captureSessionManager: CaptureSessionManager, didReceiveImage image: CIImage) { }
}
...
final public class CaptureSessionManager: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
  ...
  private let completeImageCapture_queue = DispatchQueue(label: "completeImageCapture_queue",
                                                         qos: .userInitiated,
                                                         autoreleaseFrequency: .workItem,
                                                         target: nil)
  private let sampleDelegateQueue = DispatchQueue(label: "video_ouput_queue",
                                                  qos: .userInitiated)
  // MARK: Life Cycle
  ...
  @discardableResult
  private func displayRectangleResult(_ result: RectangleDetectorResult) -> Quadrilateral {
        ...
        benchmark(operationTitle: "Photo capture") {
          ...
          guard let image = UIImage(data: imageData) else {
            DispatchQueue.main.async {
              ...
              strongSelf.delegate?
                .captureSessionManager(strongSelf,
          ...
          if let displayedRectangleResult = self?.displayedRectangleResult {
            ...
            quad = quad?.scale(displayedRectangleResult.imageSize,
                               orientedImage.size,
