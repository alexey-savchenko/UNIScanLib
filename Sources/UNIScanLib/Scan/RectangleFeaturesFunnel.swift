...
final class RectangleFeaturesFunnel {
  ...
  func add(
    ...
    completion: (AddResult, Quadrilateral) -> Void
  ) {
    ...
    if let previousRectangle = currentRectangle,
       bestRectangle.rectangleFeature.isWithin(
         autoScanMatchingThreshold,
         ofRectangleFeature: previousRectangle
       ) {
  ...
  private func breakTie(
    ...
    if rect1.rectangleFeature.isWithin(matchingThreshold, ofRectangleFeature: currentRectangle) {
      ...
      matchingThreshold,
      ofRectangleFeature: currentRectangle
    ) {
      return rect2
    }
